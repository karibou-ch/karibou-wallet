...
class Wallet {
  ...
  updateAvailable(wid,available,callback) {
    var promise = new Promise;
  ...
  updateTransfer (wid,available,callback) {
    var promise = new Promise;
    if(callback){promise.addBack(callback);}
    assert(wid);
    assert(available);
...
Wallet.statics.__retrieve=async function (query) {
  const wallet = null;
  ...
  if(wallet.locked){
    throw (new Error("The wallet is already running another task"));      
  }

  //
  // check apikey of this document
  if(config.option('apikey')!==wallet.apikey){
    throw (new Error("This wallet does not belongs to this instance"))
  }

  if(!wallet.verifySign()){
    throw (new Error("This wallet is inconsistent (GRAVE)"))

  }

  if(wallet.balance===undefined||wallet.balance===null){
    throw (new Error("Le service de paiement est incomplet (3)"));
  }
  //
...
Wallet.statics.retrieve = async function (wid) {
  assert(wid);
...
Wallet.statics.retrieveOneGift = async function (number) {

  //
  // filter with your apikey
  const apikey=config.option('apikey');

  //TODO should we must pad the number if it's <16 of len?

  const wallet = await this.__retrieve({'card.number':number,apikey:apikey,giftcode:true});
  if(wallet.card.registeredTo){
    throw (new Error("Impossible d'afficher cette carte!"));
  }
...
Wallet.statics.retrieveAllGift = async function (filter) {
  //
  // filter with your apikey
  const apikey=config.option('apikey');
  const query=Object.assign({},filter||{},{apikey:apikey,giftcode:true});
...
//
// Retrieves the giftcode
Wallet.statics.retrieveAll = async function (filter,callback) {
  const apikey=config.option('apikey');
  const query=Object.assign({},filter||{},{apikey:apikey,giftcode:false});
...
Wallet.statics.transaction_charge=async function(wid,transaction){
  assert(wid);
  assert(transaction);
  ...
  let money=new Money(transaction.amount, Money.CHF), query={wid:wid};
  ...
  const wallet = await this.__retrieve(query);

  //
  // check date
  if(!wallet.card.expiry){
    throw (new Error("Le service de paiement n'est plus disponible (1)"));
  }
  ...
  if(ed<now){
    throw (new Error("Le service de paiement n'est plus disponible (2)"));
  }

  // amount should not be negative
  if(transactionOptions.amount<0){
    throw (new Error("Le montant n'est pas valide"));      
  }

  if(transactionOptions.amount>config.option('allowMaxAmount')){
    throw (new Error("Les transactions sont limitées à "+config.option('allowMaxAmount')/100+' CHF'));
  }

  if(_.findWhere(wallet.transactions,{id:transactionOptions.id})){
    throw (new Error("Une erreur grave c'est produite avec le service de paiement (1)"));
  }
...
Wallet.statics.transaction_capture=async function (wid, transaction,callback) {
  assert(wid);
  assert(transaction);
  ...
  const wallet = await this.__retrieve({wid:wid});

  //
  // looking for the transaction id
  const trans=wallet.transactions.find(tr => tr.id==transaction.id);
  if(!trans||!trans.id){
    throw (new Error("La transaction recherchée n'existe pas"));
  }

  if(trans.limited<now){
    throw (new Error("La transaction n'est plus valide depuis le "+trans.limited));
  }

  if(trans.status!=='authorize'){
    throw (new Error("Impossible de capturer une transaction avec le status "+trans.status));
  }
  ...
  if(transaction.amount&&transaction.amount>trans.amount){
    throw (new Error("Le montant capturé ne peut pas dépasser la somme réservée !"));
  }
...
//
// refund transaction with this account
Wallet.statics.transaction_refund=async function (wid, transaction,callback) {
  assert(wid);
  assert(transaction);
  assert(transaction.id);
  ...
  if(transaction.amount&& transaction.amount>trans.amount){
    throw (new Error("Le montant remboursé ne peut pas dépasser la somme capturée !"));
  }
...
Wallet.statics.transaction_cancel=async function (wid, transaction,callback) {
  assert(wid);
  assert(transaction);
  assert(transaction.id);
  const now = new Date();
...
//
// cancel transaction with this account
Wallet.statics.transaction_get=async function (wid, transaction_id,callback) {
  assert(wid);
...
Wallet.statics.transfer_create=async function (wid, transfer,account) {
  assert(wid);
  assert(transfer);
  assert(account);

  const now=new Date(), isCredit=(transfer.type==='credit');
  const money=new Money(transfer.amount, Money.CHF),
  var recipient_w;
  ...
  if(isCredit && ((!account.iban&&!account.account)||!account.name)) {
    throw (new Error("La provenance bancaire de votre transfert n'est pas valide"));    
  }
  ...
  const recipient= await this.__retrieve({wid:account});

  //
  // looking for the transfer id
  if(wallet.transfers.find(trans => trans.id==transfer.id)){
    throw (new Error("Une erreur grave c'est produite avec le service de paiement (2)"));
  }
  ...
  if(account===wid){
    throw (new Error("You have to specify a recipient different than your wallet"))        
  }
